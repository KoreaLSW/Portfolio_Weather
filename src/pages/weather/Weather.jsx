...
export default function Weather() {
  ...
  const handleOnclick = (index) => {
    setCount(0);
    setWeatherList("");
    if (index === 0 && unixTimeClock(weather.data.list[0].dt) !== "00:00:00") {
    ...
    for (let i = index; i < weather.data.list.length; i++) {
      if (i >= 0) {
        if (unixTimeClock(weather.data.list[i].dt) === "00:00:00") {
          setWeatherList("");
          for (let j = i; j < i + 8; j++) {
            setWeatherList((prev) => [...prev, weather.data.list[j]]);
          }
      ...
      <ul className={styles.weather_item}>
        {weather &&
          weather.data.list.map(
            (value, index) =>
              (unixTimeClock(value.dt) === "00:00:00" || index === 0) && (
                <WeatherItem
                  key={value.dt}
                  item={value}
                  index={index}
