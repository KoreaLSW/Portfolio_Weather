...
import { lat, locationlat, locationlon, lon } from "../../api/weatherAPIs";
...
export default function Location() {
  ...
  useEffect(() => {
    ...
    searchDetailAddrFromCoords(initialLatLng, function (result, status) {
      if (status === kakao.maps.services.Status.OK) {
        const initialAddr = !!result[0].road_address
          ? result[0].road_address.address_name
          : result[0].address.address_name;
        setAddress(initialAddr);
        setLocation({ La: initialLatLng.getLng(), Ma: initialLatLng.getLat() });
      }
    });
